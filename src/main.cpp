...
int main(int argc, char** argv) 
{
    ...
    auto server = MQTT_NS::server<>(
        boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), boost::lexical_cast<std::uint16_t>(argv[1]) ), ioc);

    server.set_error_handler([](MQTT_NS::error_code ec) 
        {
    ...
    server.set_accept_handler(
        [&connections, &subs](con_sp_t spep)
        {
            ...
            ep.set_error_handler(
                [&connections, &subs, wp] (MQTT_NS::error_code ec) 
                {
            ...
            ep.set_connect_handler([&connections, wp]
                (MQTT_NS::buffer client_id, MQTT_NS::optional<MQTT_NS::buffer> username, 
            ...
            ep.set_publish_handler([&subs]
                (MQTT_NS::optional<packet_id_t> packet_id, MQTT_NS::publish_options pubopts, 
                    MQTT_NS::buffer topic_name, MQTT_NS::buffer contents) 
                {
                    ...
                    {
                        r.first->con->publish(topic_name, contents, std::min(r.first->qos_value, pubopts.get_qos()));
                    }
                    return true;
                });
            ...
            ep.set_subscribe_handler(
                [&subs, wp] (packet_id_t packet_id, std::vector<MQTT_NS::subscribe_entry> entries) 
                {
                    ...
                    sp->publish("test", "THIS IS A TEST MESSAGE", mqtt::qos::at_most_once);
                    return true;
                }
            );

            ep.set_unsubscribe_handler([&subs, wp](packet_id_t packet_id, std::vector<MQTT_NS::unsubscribe_entry> entries) 
                {
